...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "SELECT file_path, image_name FROM tbl_home_hero WHERE id = $id";
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadFile)) {
            ...
            if ($current_image_name !== $new_image_name) {
                $updateImageNameQuery = "UPDATE tbl_home_hero SET image_name = '$new_image_name' WHERE id = '$id'";
                mysqli_query($conn, $updateImageNameQuery);
            }
        } else {
            $_SESSION['profile-error'] = "File upload failed!";
            header('Location: dev.edit.hhero.php?id=' . $id);
            exit;
        }
    }
    ...
    if (mysqli_query($conn, $updateQuery)) {
        ...
        $_SESSION['profile-success'] = "Hero section updated successfully!";
    } else {
        $_SESSION['profile-error'] = "Failed to update hero section!";
    }
